...
import {
  IExtractAccount,
  IPayloadAccountBalance,
} from "../../src/interfaces/account/IAccount";

export const accountModelMocked: jest.Mocked<IAccountRepository> = {
  create: jest.fn<() => Promise<void>>().mockResolvedValue(),
  consultBalance: jest
  ...
  extractAccountBetweenDate: jest
    ...
    .mockResolvedValue([
      { type: "deposit", amount: 100, created_at: "2024-07-01" },
    ]),
  statusChange: jest.fn<() => Promise<void>>().mockResolvedValue(),
};

export const carrierModelMocked: jest.Mocked<ICarrierRepository> = {
  create: jest.fn<() => void>().mockReturnValue(),
  statusChange: jest.fn<() => void>().mockReturnValue(),
};
